...
export function HotTubGuideChat() {
  ...
  // ✅ Determine context-based greeting
  useEffect(() => {
    if (pathname?.startsWith("/models/")) {
      ...
      setGreeting("Would you like me to highlight key differences between these hot tubs?");
    } else {
      setGreeting("");
    }
  }, [pathname]);

  // ✅ Only render on product-related pages
  if (!pathname?.startsWith("/models") && pathname !== "/compare") return null;
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-0 right-0 left-0 md:right-6 md:left-auto md:w-[380px] h-[50vh] md:h-[40vh] bg-gradient-to-b from-blue-50 to-white border-t md:border border-gray-200 rounded-t-2xl md:rounded-2xl shadow-2xl z-50 overflow-hidden flex flex-col"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 text-sm text-gray-700">
              <div className="bg-blue-100 text-gray-800 p-3 rounded-lg inline-block max-w-[85%] shadow-sm">
                {greeting || "Hi there! How can I help you choose your perfect hot tub?"}
              </div>

              {/* Placeholder AI response area */}
              <p className="text-gray-500 text-xs mt-4">
                (Future: this will connect to the AI assistant.)
              </p>
            </div>
